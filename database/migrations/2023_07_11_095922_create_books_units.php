...
class CreateBooksUnits extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('books_units', function (Blueprint $table) {
            $table->id('unitsId')->comment('Id');
            $table->integer('booksId')->comment('書籍Id');
            $table->string('unitsName')->comment('單元名稱');
            $table->string('unitsCode')->comment('單元代碼');
        ...
        DB::statement("ALTER TABLE `books_units` comment '書籍單元'");
    }
    ...
    {
        Schema::dropIfExists('books_units');
    }
}
